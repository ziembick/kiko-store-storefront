...
test.describe("Testes de endereços", () => {
  test("Criar um novo endereço é exibido durante o checkout", async ({
    ...
    await test.step("Navegar até o modal de novo endereço", async () => {
    ...
    await test.step("Preencher e salvar o novo endereço", async () => {
      const modal = addressesPage.addAddressModal
      await modal.firstNameInput.fill("Primeiro")
      await modal.lastNameInput.fill("Último")
      await modal.companyInput.fill("PrimeiraCorp")
      await modal.address1Input.fill("123 Rua Falsa")
      await modal.address2Input.fill("Apt 1")
      await modal.postalCodeInput.fill("11111")
      await modal.cityInput.fill("Cidade")
    ...
    await test.step("Navegar para uma página de produto e adicionar um produto ao carrinho", async () => {
    ...
    await test.step("Verificar se o endereço está correto no processo de checkout", async () => {
      await checkoutPage.selectSavedAddress("123 Rua Falsa")
      await expect(checkoutPage.shippingFirstNameInput).toHaveValue("Primeiro")
      await expect(checkoutPage.shippingLastNameInput).toHaveValue("Último")
      await expect(checkoutPage.shippingCompanyInput).toHaveValue("PrimeiraCorp")
      ...
      await expect(checkoutPage.shippingCityInput).toHaveValue("Cidade")
      ...
      ).toContain("Estados Unidos")
    })
  })

  test("Realizar todas as ações de CRUD para um endereço", async ({
    accountAddressesPage: addressesPage,
  }) => {
    await test.step("Navegar até o modal de novo endereço", async () => {
      await addressesPage.goto()
    ...
    await test.step("Preencher e salvar um novo endereço", async () => {
      const { addAddressModal } = addressesPage
      await addAddressModal.firstNameInput.fill("Primeiro")
      await addAddressModal.lastNameInput.fill("Último")
      await addAddressModal.companyInput.fill("MinhaCorp")
      await addAddressModal.address1Input.fill("123 Rua Falsa")
      ...
      await addAddressModal.countrySelect.selectOption({ label: "Estados Unidos" })
    ...
    await test.step("Verificar se o contêiner do endereço foi adicionado à página", async () => {
      addressContainer = addressesPage.getAddressContainer("Primeiro Último")
      await expect(addressContainer.name).toHaveText("Primeiro Último")
      await expect(addressContainer.company).toHaveText("MinhaCorp")
      await expect(addressContainer.address).toContainText("123 Rua Falsa")
    ...
    await test.step("Atualizar a página e verificar se o endereço foi salvo", async () => {
    ...
    await test.step("Editar o endereço", async () => {
      await addressContainer.editButton.click()
      await addressesPage.editAddressModal.container.waitFor({ state: "visible" })
      await addressesPage.editAddressModal.firstNameInput.fill("Segundo")
      await addressesPage.editAddressModal.lastNameInput.fill("Final")
      await addressesPage.editAddressModal.companyInput.fill("MinhaOutraCorp")
      await addressesPage.editAddressModal.address1Input.fill("123 Rua Nova")
      await addressesPage.editAddressModal.address2Input.fill("Unidade 3")
    ...
    await test.step("Verificar se as edições foram salvas no contêiner do endereço", async () => {
      addressContainer = addressesPage.getAddressContainer("Segundo Final")
      await expect(addressContainer.name).toContainText("Segundo Final")
      await expect(addressContainer.company).toContainText("MinhaOutraCorp")
      await expect(addressContainer.address).toContainText("123 Rua Nova, Unidade 3")
    ...
    await test.step("Atualizar a página e verificar se as edições foram salvas", async () => {
      await addressesPage.page.reload()
      await expect(addressContainer.name).toContainText("Segundo Final")
      await expect(addressContainer.company).toContainText("MinhaOutraCorp")
      await expect(addressContainer.address).toContainText("123 Rua Nova, Unidade 3")
    ...
    await test.step("Excluir o endereço", async () => {
    ...
    await test.step("Verificar se o endereço permanece excluído após a atualização", async () => {
      await addressesPage.page.reload()
      await expect(addressContainer.container).not.toBeVisible()
    })
  })
