...
test.describe("Testes da página de pedidos da conta", async () => {
  test.beforeEach(async ({ accountAddressesPage }) => {
    ...
    await test.step("Adicionar endereço padrão", async () => {
  ...
  test("Verificar que a página de pedidos exibe um contêiner vazio", async ({
    accountOrdersPage,
  }) => {
  ...
  test("Pedido aparece após o fluxo de checkout", async ({
    accountOrdersPage,
    accountOrderPage,
    cartPage,
    checkoutPage,
    ...
    await test.step("Navegar para a página de um produto", async () => {
    ...
    await test.step("Adicionar o produto ao carrinho e ir para o checkout", async () => {
    ...
    await test.step("Preencher a primeira etapa do processo de checkout", async () => {
      await checkoutPage.selectSavedAddress("123 Rua Falsa")
      await checkoutPage.shippingEmailInput.fill("teste@exemplo.com")
    ...
    await test.step("Completar o restante do processo de pagamento", async () => {
      await checkoutPage.selectDeliveryOption("FakeEx Padrão")
    ...
    let orderId = ""
    await test.step("Verificar que as informações da página de pedidos estão corretas", async () => {
      orderId = (await publicOrderPage.orderId.textContent()) || ""

      await test.step("Verificar que os produtos pedidos estão corretos", async () => {
        const product = await publicOrderPage.getProduct("Moletom", "M")
        await expect(product.name).toContainText("Moletom")
        await expect(product.variant).toContainText("M")
        await expect(product.quantity).toContainText("1")
      })

      await test.step("Verificar que as informações de envio estão corretas", async () => {
        const address = publicOrderPage.shippingAddressSummary
        await expect(address).toContainText("Primeiro")
        await expect(address).toContainText("Último")
        await expect(address).toContainText("123 Rua Falsa")
        await expect(address).toContainText("11111")
        await expect(address).toContainText("Cidade")
        await expect(address).toContainText("US")

        const contact = publicOrderPage.shippingContactSummary
        await expect(contact).toContainText("teste@exemplo.com")
        await expect(contact).toContainText("3031112222")

        const method = publicOrderPage.shippingMethodSummary
        await expect(method).toContainText("FakeEx Padrão")
      })
    })

    await test.step("Verificar que a página de pedidos da conta exibe um resultado", async () => {
      await accountOrdersPage.goto()
      const order = await accountOrdersPage.getOrderById(orderId)
      expect(order.items.length).toBe(1)
      expect(order.items[0].title).toContainText("Moletom")
    ...
    await test.step("Verificar que a página do pedido exibe as informações corretas", async () => {
      await test.step("Verificar que o ID do pedido está correto", async () => {
        await expect(accountOrderPage.orderId).toHaveText(orderId)
      })

      await test.step("Verificar que os produtos pedidos estão corretos", async () => {
        const product = await accountOrderPage.getProduct("Moletom", "M")
        await expect(product.name).toContainText("Moletom")
        await expect(product.variant).toContainText("M")
        await expect(product.quantity).toContainText("1")
      })
      ...
      await test.step("Verificar que as informações de envio estão corretas", async () => {
        ...
        await expect(contact.getByText("teste@exemplo.com")).toBeVisible()
    ...
    await test.step(
      "Navegar de volta para a página de pedidos e verificar que o botão de voltar funciona",
      async () => {
